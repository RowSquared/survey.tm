...
test_sheet <- get_tms_data(ss = test_sheet_id)

# Test 1: Check if the function returns a named list of the expected length
test_that("get_tms_data returns a named list of the expected length", {
...
test_that("get_tms_data returns a named list containing data.tables", {
...
test_that("get_tms_data returns the correct sheets", {
...
test_that("get_tms_data returns an error with an invalid Google Sheet ID", {
  expect_error(get_tms_data(ss = "invalid_id"))
})

# Test 5: Check if the function returns an error when provided with non-existent sheet names
test_that("get_tms_data returns an error with non-existent sheet names", {
  expect_error(get_tms_data(ss = test_sheet_id, sheets = c("non_existent_sheet")))
})
