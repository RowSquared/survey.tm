...
3.  **Add more sheets** for additional languages as needed, naming each
    one appropriately.

After setting up your “Translation Database Sheet” using either option,
ensure you copy and save the Google Sheet ID for future use in the
workflow.
...
R environment to begin. It’s recommended to examine each object after
each step to gain insight into the process and understand the changes
taking place.
...
parse_suso_titems()`) against the 'Translation Database' list returned by`get_tdb_data()\`.
